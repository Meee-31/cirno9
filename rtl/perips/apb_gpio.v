...
(
    ...
    always @(posedge HCLK or negedge HRESETn) begin
        if (~HRESETn) begin
            o_interrupt <= 1'b0;
            r_status  <= 'h0;
	end else if (!o_interrupt && s_rise_int) begin //rise interrupt if not already rise
            o_interrupt <= 1'b1;
            r_status  <= s_is_int_all;
	end else if ((((o_interrupt && PSEL) && PENABLE) && !PWRITE) && (s_apb_addr == `REG_INTSTATUS)) begin    //clears int if status is read
            o_interrupt <= 1'b0;
            r_status  <= 'h0;
        end
    end
